...
public class DatabaseHelper {
    ...
    /**
     * Gets the assignment of a course by its course UUID
     *
     * @param courseId Course UUID
     * @return List of assignments of the course
     */
    ...
    public static void deleteCourse(UUID courseId, UUID userId) {
        Connection connection = null;
        try {
            // create a database connection
            connection = DriverManager.getConnection("jdbc:sqlite:cocon.db");
            ...
            final String query = "SELECT COUNT(" + KEY_ID + ", " + KEY_COURSE_ID + ") FROM\n" +
                    "  " + TABLE_USER
                    + " WHERE " + KEY_ID + " = '" + userId + "' AND " +
                    KEY_COURSE_ID + " = '" + courseId + "';";
            ResultSet rs = statement.executeQuery(query);
            rs.next();
            final int count = rs.getInt(0);
            ...
            if (count > 0) {
                ...
                rs = statement.executeQuery(deleteQuery);
                
                // Check if there are still users for this course
                final String queryUsers =
                        "SELECT COUNT(" + KEY_ID + "," + KEY_COURSE_ID + ")" +
                                "\n" +
                                "FROM " + TABLE_USER +
                                "\n" +
                                "WHERE " + KEY_COURSE_ID + " = '" + courseId + "';";
                
                rs = statement.executeQuery(queryUsers);
                rs.next();
                final int numUsers = rs.getInt(0);
                
                // If course is not in use anymore, delete the course
                if (numUsers == 0) {
                    List<Assignment> assignments = getAssignmentsOfCourse
                            (courseId);
                    
                    // Delete its assignments
                    assignments.forEach(assignment -> deleteAssignment
                            (assignment.getId()));
                    
                    
                    // Delelte the actual course
                    final String deleteCourseQuery =
                            "DELETE FROM " + TABLE_COURSES + " WHERE " +
                                    KEY_COURSE_ID + " = '" + courseId + "';";
                    rs = statement.executeQuery(deleteCourseQuery);
    ...
    public static void deleteAssignment(UUID assignmentId) {
        Connection connection = null;
        try {
            // create a database connection
            connection = DriverManager.getConnection("jdbc:sqlite:cocon.db");
            ...
            final String deleteQuery =
                    "DELETE FROM " + TABLE_ASSIGNMENTS + " WHERE " +
                            KEY_ID + " = '" + assignmentId + "';";
